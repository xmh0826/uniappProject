<template>
	<!-- SKU 弹窗组件 -->
	<vk-data-goods-sku-popup v-model="isShowSku" :localdata="localdata" :mode="mode" ref="skuPopupRef" :actived-style="{
			color:'#27BA9b',
			borderColor:'#27BA9b',
			backgroundColor:'#E9F8F5'
		}" @add-cart="onAddCart" @buy-now="onBuyNow"></vk-data-goods-sku-popup>
	<scroll-view scroll-y class="viewport">
		...
		<view class="detail panel">
			...
			<view class="content">
				...
				<!-- 图片详情 -->
				<image mode="widthFix" v-for="item in goods?.details.pictures" :key="item" :src="item">
				</image>
			</view>
		</view>
	...
	<uni-popup ref="popup" type="bottom" background-color="#fff">
		<AddressPanel v-if="popupName === 'address'" @close="popup?.close()" :list="addressList"></AddressPanel>
		<ServicePanel v-else @close="popup?.close()"></ServicePanel>
	</uni-popup>
</template>
...
<script setup lang="ts">
	...
	import { SkuPopupEvent, SkuPopupInstance, SkuPopupLocaldata } from '../../types/vk-data-goods-sku-popup';
	...
	const selectArrText = computed(() => {
		// trim: 去除字符串头尾处的空格
		return skuPopupRef.value?.selectArr?.trim() || '请选择商品规格'
	})
