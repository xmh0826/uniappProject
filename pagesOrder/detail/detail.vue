<script setup lang="ts">
	...
	// 获取当前页面实例，就是数组最后一项  at 提取数组某一项，且可以从最后一项选择（- 代表反过来）
	const pageInstance = pages.at(-1) as any

	// 页面动画效果都得等页面渲染完毕后， 绑定动画效果
	onReady(() => {
		// 动画效果   uniapp 官方对 animate类型 处理不完善，故会报错，所以需屏蔽错误，通过断言屏蔽
		pageInstance.animate('.navbar', [{
			backgroundColor: 'transparent'  // 第一个动画
		}, {
			backgroundColor: '#f8f8f8'      // 第二个动画
		}], 1000, {
			scrollSource: '#scroller',  // id名称必须与scroll-view 对应
			timeRange: 2000,
			startScrollOffset: 0,       // 进入到此距离开始执行第二个动画
			endScrollOffset: 100,       // 进入到第二个距离后执行完毕第二个动画
		})
	})
	...
	const getMemberOrderByIdData = async () => {
		...
		//  如果  order.value.orderState 中的值含有数组中的值 则为true                     
	...
	const onOrderSend = async () => {
		// 因为在打包的时候如果不是开发环境就不会生成这段if代码，所以虽然已经判断过是否为开发环境了还这么写
		if (isDev) {
				...
				<template v-if="order.orderState === OrderState.DaiFuKuan">
					...
					<view class="tips">
						...
						<uni-countdown :second="order.countdown" @timeup="onTimeup" color="#fff" splitor-color="#fff"
							:show-day="false" :show-colon="false"></uni-countdown>
					</view>
